...
| `foxclaw scan` | `--profile`, `--ruleset`, `--json`, `--ndjson`, `--ecs`, `--output`, `--ndjson-out`, `--ecs-out`, `--sarif-out`, `--snapshot-out`, `--history-db`, `--learning-artifact-out` | `0` clean, `1` operational, `2` high findings | JSON/vendor-neutral NDJSON/ECS NDJSON/SARIF/snapshot files and optional learning artifact | deterministic ordering in renderers; timestamps volatile unless deterministic mode or pinned inputs (`foxclaw/report/jsonout.py:10-13`, `foxclaw/report/siem.py:50-99`, `foxclaw/report/ecs.py:33-78`, `foxclaw/report/sarif.py:25-40`, `foxclaw/report/snapshot.py:49-81`) |
...
| `foxclaw acquire windows-share-scan` | `--source-profile`, `--staging-root`, `--output-dir`, `--ecs-out`, `--treat-high-findings-as-success` | returns scan code (`0`/`2`) or `1` for operational errors; optional normalization of `2 -> 0` | `foxclaw.json`, optional `foxclaw.ecs.ndjson`, `foxclaw.sarif`, `foxclaw.snapshot.json`, `stage-manifest.json` | deterministic staged manifest shape and explicit command capture |
| `foxclaw acquire windows-share-batch` | `--source-root`, `--staging-root`, `--out-root`, `--include-profile-name`, `--exclude-profile-name`, `--workers`, `--profile-timeout-seconds` | `1` if any operational failure, else `2` if findings present, else `0` | `windows-share-batch-summary.json` plus per-profile output directories | directory enumeration and summary sorted by profile; include/exclude policy is name-exact and deterministic (`foxclaw/acquire/windows_share_batch.py`) |
