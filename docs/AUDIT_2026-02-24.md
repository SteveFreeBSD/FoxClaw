...
| | |
|---|---|
| **File** | [windows_share.py:392-394](../foxclaw/acquire/windows_share.py#L392-L394) |
| **Impact** | CI/pipelines misclassify config failures as "scan completed with HIGH findings" |

`--intel-snapshot-id` without `--intel-store-dir` returns exit code `2`. Per the documented contract ([README.md](../README.md), [ARCHITECTURE.md](../docs/ARCHITECTURE.md)), exit `1` = operational error, exit `2` = HIGH findings present.
...
| | |
|---|---|
| **File (guard present)** | [cli.py:202-208](../foxclaw/cli.py#L202-L208) |
| **File (guard absent)** | [cli.py:447-465](../foxclaw/cli.py#L447-L465) |
| **Architecture invariant** | [ARCHITECTURE.md](../docs/ARCHITECTURE.md) — "UNC direct scanning disabled by default" |
...
| **Exclude config** | [check_secrets.sh:12](../scripts/check_secrets.sh#L12) |
...
| Location | Tuple |
|---|---|
| [windows_share.py:20](../foxclaw/acquire/windows_share.py#L20) | `("parent.lock", ".parentlock", "lock")` |
| [profiles.py:12](../foxclaw/profiles.py#L12) | `("parent.lock", "lock")` |
| [scan.py:34](../foxclaw/scan.py#L34) | `("parent.lock", "lock")` |
...
| | |
|---|---|
| **Code** | [history.py:257-260](../foxclaw/learning/history.py#L257-L260) (docstring says "deterministic") |
| **Timestamp** | [history.py:310](../foxclaw/learning/history.py#L310) (`datetime.now(UTC)`) |
| **Doc claim** | [SCAN_LEARNING_LOOP.md](../docs/SCAN_LEARNING_LOOP.md) |
...
| Line | Check | Effect |
|---|---|---|
| [79](../foxclaw/learning/history.py#L79) | `hasattr(evidence, 'firefox_version')` | Always `False` → `firefox_version` is always `None` |
| [93](../foxclaw/learning/history.py#L93) | `hasattr(evidence, 'artifacts')` | Field exists on `EvidenceBundle`; guard is redundant |
| [103](../foxclaw/learning/history.py#L103) | `hasattr(evidence, 'ruleset_name')` | Always `False` → always returns `(None, None)` |
...
| Issue | Details |
|---|---|
| WS-55A marked pending | Implemented in code ([history.py](../foxclaw/learning/history.py), [cli.py:182](../foxclaw/cli.py#L182)) but docs still say in-progress: [PREMERGE_READINESS.md:66](../docs/PREMERGE_READINESS.md), [WORKSLICES.md:81](../docs/WORKSLICES.md), [SCAN_LEARNING_LOOP.md:20](../docs/SCAN_LEARNING_LOOP.md) |
| Malformed table | [WORKSLICES.md:81](../docs/WORKSLICES.md) contains `|\n|` embedded in table text |
| HSTS artifact name mismatch | Code uses `SiteSecurityServiceState.bin` ([artifacts.py:17](../foxclaw/collect/artifacts.py#L17)); docs reference `.txt` ([SECURITY_MODEL.md:78](../docs/SECURITY_MODEL.md), [WORKSLICES.md:79](../docs/WORKSLICES.md)) |
| Exit code 2 contract too narrow | README says exit `2` = "HIGH findings". But `2` is also returned for snapshot drift ([cli.py:974](../foxclaw/cli.py#L974)) and suppression audit violations ([cli.py:1123,1140](../foxclaw/cli.py#L1123)) |
...
| File | Line |
|---|---|
| [engine.py](../foxclaw/rules/engine.py#L16) | 16 |
| [correlation.py](../foxclaw/intel/correlation.py#L18) | 18 |
| [sarif.py](../foxclaw/report/sarif.py#L18) | 18 |
| [snapshot.py](../foxclaw/report/snapshot.py#L15) | 15 |
| [snapshot_diff.py](../foxclaw/report/snapshot_diff.py#L20) | 20 |
| [fleet.py](../foxclaw/report/fleet.py#L21) | 21 |

Three copies of `_table_exists()`:

| File | Line |
|---|---|
| [blocklist.py](../foxclaw/intel/blocklist.py#L61) | 61 |
| [correlation.py](../foxclaw/intel/correlation.py#L443) | 443 |
| [reputation.py](../foxclaw/intel/reputation.py#L133) | 133 |

Two copies of `_sqlite_ro_uri()`:

| File | Line |
|---|---|
| [credentials.py](../foxclaw/collect/credentials.py#L146) | 146 |
| [sqlite.py](../foxclaw/collect/sqlite.py#L71) | 71 |
...
- [windows_share.py:119](../foxclaw/acquire/windows_share.py#L119)
- [cli.py:609](../foxclaw/cli.py#L609)
- [WINDOWS_SHARE_TESTING.md:119](../docs/WINDOWS_SHARE_TESTING.md)
...
[policies.py:92-93](../foxclaw/collect/policies.py#L92-L93) raises `ProfilePathSymlinkError` with "profile path" wording when validating `--policy-path` inputs.
...
[keyring.py:12-17](../foxclaw/rules/keyring.py#L12-L17) imports private functions from `trust.py`:
...
| # | Sev | Fix | Status |
|---|---|---|---|
| C-1 | Critical | Change exit code `2` → `1` in `windows_share.py:394` | Fixed (PR #11) |
| H-1 | High | Add UNC guard in `live` command | Fixed (PR #11) |
| H-2 | High | Update `check_secrets.sh` exclude patterns | Fixed (PR #11) |
| M-1 | Medium | `ruff --fix tests/test_scan_history.py` | Fixed (PR #11) |
| M-2 | Medium | Consolidate lock marker constant | Fixed (PR #11) |
| M-3 | Medium | Clarify determinism claim in `history.py` | Fixed (PR #11) |
| M-4 | Medium | Fix `hasattr()` dead code in `history.py` | Fixed (PR #11) |
| M-5 | Medium | Update docs (WS-55A status, HSTS name, exit code contract) | Fixed (WS-61) |
| M-6 | Medium | Consolidate duplicated constants/helpers | Fixed (WS-62) |
| L-1 | Low | Decide on `--keep-stage-writeable` spelling | Fixed (WS-63) |
| L-2 | Low | Fix misleading symlink error message in `policies.py` | Fixed (WS-63) |
| L-3 | Low | Promote private trust functions to public API | Fixed (WS-63) |
