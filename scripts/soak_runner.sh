...
Options:
  ...
  --siem-elastic-fleet-runs <N> Elastic Fleet ECS smoke runs per cycle (default: 0).
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --siem-elastic-fleet-runs) SIEM_ELASTIC_FLEET_RUNS="${2:-}"; shift 2 ;;
...
siem_elastic_fleet_runs_per_cycle=${SIEM_ELASTIC_FLEET_RUNS}
...
while true; do
  ...
  for ((s=1; s<=SIEM_ELASTIC_FLEET_RUNS; s++)); do
    siem_cycle_dir="${RUN_DIR}/siem-elastic-fleet/cycle-${cycle}-run-${s}"
    mkdir -p "${siem_cycle_dir}"
    run_step_cmd "${cycle}" "siem_elastic_fleet" "${s}" "${LOG_DIR}/cycle-${cycle}-siem-elastic-fleet-${s}.log" "${siem_cycle_dir}" \
      "${PYTHON_BIN}" "${ROOT_DIR}/scripts/siem_elastic_fleet_smoke.py" \
      --output-dir "${siem_cycle_dir}" \
      --python-bin "${PYTHON_BIN}" || overall_fail=1
    if [[ "${stop_requested}" -eq 1 ]]; then
      break
